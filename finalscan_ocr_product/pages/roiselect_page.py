...
class ROISlectPage:
    def __init__(self, parent, config_folder="config", config_file="config/camera_config.json", config_check_interval=1000):
        # Create a new top-level window (popup)
        self.window = tk.Toplevel(parent)
        self.window.title("Camera Feed with ROI")
        self.window.geometry("640x480")  # Set the window size
        ...
        self.cancel_button = tk.Button(self.window, text="Cancel", command=self.on_cancel)
        self.cancel_button_window = self.canvas.create_window(480, 440, window=self.cancel_button)  # Place at (480, 440)

        self.confirm_button = tk.Button(self.window, text="Confirm", command=self.save_roi, state=tk.DISABLED)
    ...
    def check_config_changes(self, interval):
        ...
        self.window.after(interval, self.check_config_changes, interval)
    ...
    def on_mouse_release(self, event):
        ...
        # Enable the Confirm button when drawing is finished
        if self.start_x != self.end_x and self.start_y != self.end_y:
            self.confirm_button.config(state=tk.NORMAL)  # Enable Confirm button
    ...
    def save_roi(self):
        ...
        try:
            ...
            self.window.destroy()  # Close the pop-up window after saving the ROI
        except Exception as e:
            messagebox.showerror("Error", f"Failed to save ROI: {e}")

    def on_cancel(self):
        """Close the window when Cancel is clicked"""
        self.window.destroy()  # Close the pop-up window
